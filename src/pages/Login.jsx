...
export const Login = () => {
  ...
  const validate = () => {
    if (signState === "Register" && !name.trim())
      return "Full name is required.";
    if (!email.trim()) return "Email address is required.";
    if (!password) return "Password is required.";
    if (signState === "Register" && password.length < 6)
      return "Password must be at least 6 characters.";
    return null;
  };
  ...
  const inputCls =
    "w-full px-4 py-3 bg-canvas-light border border-hairline rounded-app-xs text-ink placeholder-mute text-[15px] focus:outline-none focus:ring-2 focus:ring-ink focus:border-transparent transition-colors";
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="flex flex-col justify-between px-8 py-10 md:px-16 bg-canvas-light">
        ...
        <div className="w-full max-w-sm mx-auto py-16">
          ...
          {fieldError && (
            <div className="flex items-start gap-3 bg-error/8 border border-error/20 rounded-app-xs px-4 py-3 mb-5">
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M10 18a8 8 0 100-16 8 8 0 000 16zm-.75-5.25a.75.75 0 001.5 0v-4a.75.75 0 00-1.5 0v4zm.75-7a.75.75 0 100 1.5.75.75 0 000-1.5z"
                  clipRule="evenodd"
                />
              </svg>
              <p className="text-error text-[13px] leading-snug">
                {fieldError}
              </p>
            </div>
          )}
          ...
          <form onSubmit={user_auth} className="space-y-4" noValidate>
            {signState === "Register" && (
              <div>
                <label className="block font-mono text-[11px] uppercase tracking-widest text-mute mb-1.5">
                  Full Name
                </label>
                <input
                  type="text"
          ...
          {signState === "Login" && (
            <div className="mt-6 rounded-r-app-sm border-l-2 border-ink bg-canvas-paper px-4 py-3">
              <p className="font-mono text-[10px] uppercase tracking-widest text-mute mb-1.5">Demo Account</p>
              <p className="text-ink text-[13px]">
                <span className="text-mute">Email:</span> demo@gmail.com
                <span className="mx-2 text-hairline">|</span>
                <span className="text-mute">Password:</span> 123456
              </p>
            </div>
          )}

          <p className="text-mute text-[14px] text-center mt-6">
            {signState === "Login"
            ...
            <button
              type="button"
              onClick={switchState}
              className="text-ink font-medium underline underline-offset-2 hover:text-graphite transition-colors"
            >
      ...
      <div className="hidden lg:block relative overflow-hidden">
        <img
          ...
          alt="A quiet reading space"
          className="absolute inset-0 w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-ink/60 via-ink/10 to-transparent" />
        <div className="absolute bottom-10 left-10 right-10">
          <p
            className="text-on-primary font-normal mb-3"
            style={{
